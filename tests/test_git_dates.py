...
import hooks.git_dates as git_dates


def test_format_git_timestamp_uses_requested_footer_format():
    formatted = git_dates.format_git_timestamp("2026-01-22T14:07:33-05:00")
...
def test_git_timestamp_passes_page_path_after_separator(monkeypatch):
    ...
    monkeypatch.setattr(git_dates.subprocess, "run", fake_run)

    result = git_dates.git_timestamp(Path("C:/repo"), Path("C:/repo/docs/index.md"))
...
def test_on_config_sets_website_updated_at(monkeypatch):
    monkeypatch.setattr(git_dates, "git_timestamp", Mock(return_value="2026-01-22T14:07:33-05:00"))
    config = {"config_file_path": "C:/repo/mkdocs.yml", "extra": {}}

    result = git_dates.on_config(config)
...
def test_on_page_markdown_sets_page_specific_updated_at(monkeypatch):
    monkeypatch.setattr(git_dates, "git_timestamp", Mock(return_value="2026-02-03T09:30:00-05:00"))
    ...
    markdown = git_dates.on_page_markdown("body", page=page, config=config, files=[])
